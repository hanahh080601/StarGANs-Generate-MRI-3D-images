...
class Generator(nn.Module):
    """Generator network."""
    def __init__(self, conv_dim=24, c_dim=4, repeat_num=6):
        super(Generator, self).__init__()

        layers = []
        layers.append(nn.Conv3d(1 + c_dim, conv_dim, kernel_size=7, stride=1, padding=3, bias=False))
    ...
    def forward(self, x, c):
        ...
        # Note that this type of label conditioning does not work at all if we use reflection padding in Conv3d.
...
class Discriminator(nn.Module):
    ...
    def __init__(self, patch_size=72, conv_dim=24, c_dim=4, repeat_num=6):
